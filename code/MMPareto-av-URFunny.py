...
from models.models import URFunnyAVClassifier
...
def train_epoch(args, epoch, model, device, dataloader, optimizer, scheduler, writer=None):
    ...
    for step, (audio_input, images, text_input, label) in tqdm(enumerate(dataloader)):
        ...
        out, out_a, out_v = model(audio_input, images.float())
...
def valid(args, model, device, dataloader):
    ...
    with torch.no_grad():
        ...
        for step, (audio_input, images, text_input, label) in tqdm(enumerate(dataloader)):
            ...
            prediction_all = model(audio_input, images.float())
...
def main():
    ...
    model = URFunnyAVClassifier(args)
    model.to(device)
