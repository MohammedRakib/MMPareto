...
from models.models import URFunnyClassifier
...
def train_epoch(args, epoch, model, device, dataloader, optimizer, scheduler, writer=None):
    ...
    for step, (audio_input, images, text_input, label) in tqdm(enumerate(dataloader)):
        ...
        out, out_a, out_v, out_t = model(audio_input, images.float(), text_input)
...
def valid(args, model, device, dataloader):
    ...
    with torch.no_grad():
        ...
        for step, (audio_input, images, text_input, label) in tqdm(enumerate(dataloader)):
            ...
            prediction_all = model(audio_input, images.float(), text_input)
    ...
    if args.train:
        ...
        for epoch in range(args.epochs):
            ...
            if acc > best_acc:
                ...
                print("Loss: {:.4f}, Acc: {:.4f}, Acc_a: {:.4f}, Acc_v: {:.4f}, Acc_t: {:.4f}".format(
                    batch_loss, acc, acc_a, acc_v, acc_t))
            else:
                print("Loss: {:.4f}, Acc: {:.4f}, Acc_a: {:.4f}, Acc_v: {:.4f}, Acc_t: {:.4f},Best Acc: {:.4f}".format(
                    batch_loss, acc, acc_a, acc_v, acc_t, best_acc))
